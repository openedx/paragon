...
The `MultiSelectDropdownFilter` and `CheckboxFilter` components expect an array of values. Thus, it is est to use them with the `includesValue` filter function.
...
<StaticQuery
  query={graphql`
    query {
      ...
      dropdownFilter: componentMetadata(displayName: { eq: "DropdownFilter" }) {
        ...ComponentDocGenData
      }
      multiSelectDropdownFilter: componentMetadata(displayName: { eq: "MultiSelectDropdownFilter" }) {
        ...ComponentDocGenData
      }
    }
  `}
  render={({ textFilter, checkboxFilter, dropdownFilter, multiSelectDropdownFilter }) => {
    return (
      <div>
        <h4>TextFilter</h4>
        ...
        <h4>DropdownFilter</h4>
        {dropdownFilter ? <PropsTable propMetaData={dropdownFilter.props} /> : null}
        <h4>MultiSelectDropdownFilter</h4>
        {multiSelectDropdownFilter ? <PropsTable propMetaData={multiSelectDropdownFilter.props} /> : null}
      </div>
    );
  }}
/>
