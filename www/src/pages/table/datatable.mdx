...
The DataTable will pass the `tableName`, `itemCount` and any extra props on to it's children. Component's outside the `DataTable` component will
need to have access to the table name in order to access table data and methods.
...
<TableContextProvider >
  ...
  >
    ...
    <DataTable.Table
      tableName='famous-cats-2'
    />
    <DataTable.EmptyTable content="No results found" tableName='famous-cats-2'/>
    <DataTable.TableFooter
      tableName='famous-cats-2'
      itemCount={7}
      isPaginated
