...
() => {
  ...
  const { isScrolledToStart, isScrolledToEnd, scrollToPrevious, scrollToNext } = useOverflowScroll({
  ...
  return (
    <>
      ...
      <div ref={setOverflowRef} aria-label="example overflow scroll container" className="d-flex">
        {items}
      </div>
    </>
  );
