  ...
  postCSS(commonPostCssPlugins)
    ...
    .then((result) => {
      ...
      postCSS([postCSSMinify()])
        ...
        .then((minifiedResult) => {
          const stylesheetMinifying = ora(`Minifying for ${capitalize(name)} stylesheet...`).start();
          fs.writeFileSync(`${outDir}/${name}.css.map`, minifiedResult.map.toString());
          fs.writeFileSync(`${outDir}/${name}.min.css`, minifiedResult.css);
          stylesheetMinifying.succeed(`Success minifying for ${capitalize(name)} stylesheet`);
        });

      const hasExistingParagonThemeOutput = fs.existsSync(`${outDir}/${paragonThemeOutputFilename}`);
      let paragonThemeOutput;
      const updatingExistingParagonThemeOutput = ora(`Updating existing theme output for ${capitalize(name)} stylesheet...`).start();
      ...
      updatingExistingParagonThemeOutput.succeed(`Success updating existing theme output for ${capitalize(name)} stylesheet`);
      const writingParagonThemeOutput = ora(`Writing outputs files for ${capitalize(name)} stylesheet....`).start();
      fs.writeFileSync(`${outDir}/${paragonThemeOutputFilename}`, `${JSON.stringify(paragonThemeOutput, null, 2)}\n`);
      writingParagonThemeOutput.succeed(`Success writing outputs files for ${capitalize(name)} stylesheet`);
    })
    .then(() => {
...
function buildScssCommand(commandArgs) {
  ...
  try {
    ...
    throw new Error(`The provided path '${corePath}' is not valid.`);
  }
}
