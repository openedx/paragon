...
const compileAndWriteStyleSheets = ({
  ...
  console.log(`${chalk.yellow.bold(`Compiling ${name} stylesheets`)}`);
  ...
  postCSS(commonPostCssPlugins)
    ...
    .then((result) => {
      ...
      /* eslint-disable-next-line no-console */
    }).then(() => console.log(`${chalk.green.bold(`${capitalize(name)} stylesheets built`)}`));
};
...
function buildScssCommand(commandArgs) {
  ...
  try {
    /* eslint-disable-next-line no-console */
    console.log(`${chalk.green.bold('Building SCSS started')}`);
